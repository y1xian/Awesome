...
class HandlerUtils {
    ...
    @VisibleForTesting
    val mRunnables = ChainedRef(mLock, null)
    ...
    constructor(callback: Handler.Callback?) {
        ...
        mExec = ExecHandler(WeakReference(callback)) // Weak referencing inside ExecHandler
    }
    ...
    class ChainedRef(var lock: Lock, val runnable: Runnable?) {
        ...
        init {
            wrapper = WeakReference(runnable)?.let { WeakRunnable(it, WeakReference(this)) }
        }
    }
}